...
function Show-SCBanner {
    ...
    # Try Figlet when available (optional dependency: Install-Module Figlet)
    $lines = $null
    $figletCmd = Get-Command Write-Figlet -ErrorAction SilentlyContinue
    $figletFontMap = @{ standard = 'standard'; slant = 'slant'; block = 'block'; small = 'big'; mini = 'banner' }
    $figletFont = $figletFontMap[$Font]
    if ($figletCmd -and $figletFont) {
        try {
            $figletOut = & $figletCmd -Message $Text -Font $figletFont -PassThru
            if ($figletOut -is [string]) {
                $lines = ($figletOut -split "`r?`n").Where({ $_.Length -ge 0 })
            } elseif ($figletOut) {
                $lines = @($figletOut)
            }
        } catch {
